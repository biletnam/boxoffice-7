...
<html>
<head>
	<title>updateMovieForm -BoxOffice</title>
...
<body>
	...
	<section>
		<div class="container">
			<div class="center">
			<h2>영화 등록 수정</h2>
			<form action="${pageContext.request.contextPath}/movie/update.do" class="form-horizontal" 
			method="post" enctype="multipart/form-data" name="movieForm" onsubmit="return check();">
			  ...
			  <div class="form-group">
			    <label for="m_title" class="col-sm-2 control-label">제목</label>
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="m_title" name="m_title" value="${movieVO.m_title}">
			    </div>
			  </div>
			  <div class="form-group">
			    <label for="m_kind" class="col-sm-2 control-label">장르</label>
			    <div class="col-sm-10">
				    <label class="checkbox-inline"><input type="checkbox" name="m_kind" value="1">드라마</label>
			  ...
			  <div class="form-group">
			    <label for="m_kind" class="col-sm-2 control-label">상영등급</label>
			    <div class="col-sm-4">
			    ...
			    <div class="col-sm-4">
				  <input type="number" class="form-control" id="m_count" name="m_count" value="${movieVO.m_count}">
			    </div>
			  </div>
			  <div class="form-group">
			    ...
			    <div class="col-sm-10 pr">
			    	<div class="dateicon">
				    	<input type="text" name="m_date" id="m_date" class="form-control" value="${movieVO.m_date}"/>
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="m_youtube" name="m_youtube" value="${movieVO.m_youtube}">
			    </div>
			  </div>
			  <div class="form-group">
			    <label for="m_contents" class="col-sm-2 control-label">줄거리</label>
			    <div class="col-sm-10">
			      <textarea class="form-control" rows="10" id="m_contents" name="m_contents">${movieVO.m_contents}</textarea>	
			    </div>
			  </div>
			  <div class="form-group">
			  ...
			  <input type="hidden" name="m_file2">
			</form>
			</div>
		</div>
	</section>
	...
	<script type="text/javascript">
		// 장르, 등급 선택	
		var m_kinds='${movieVO.m_kind}';
		var m_grade=${movieVO.m_grade}-1;
		console.log("장르"+m_kinds+"등급"+m_grade);
		
		var m_kind=m_kinds.split(',');
		console.log("분할 수"+m_kind.length);
		
		for(var i=0; i<m_kind.length; i++){
			if(m_kind[i]=='1'){$('input[name=m_kind]:eq(0)').attr('checked','checked');};
			if(m_kind[i]=='2'){$('input[name=m_kind]:eq(1)').attr('checked','checked');};
			if(m_kind[i]=='3'){$('input[name=m_kind]:eq(2)').attr('checked','checked');};
			if(m_kind[i]=='4'){$('input[name=m_kind]:eq(3)').attr('checked','checked');};
			if(m_kind[i]=='5'){$('input[name=m_kind]:eq(4)').attr('checked','checked');};
			if(m_kind[i]=='6'){$('input[name=m_kind]:eq(5)').attr('checked','checked');};
			if(m_kind[i]=='7'){$('input[name=m_kind]:eq(6)').attr('checked','checked');};
			if(m_kind[i]=='8'){$('input[name=m_kind]:eq(7)').attr('checked','checked');};
		}

		$("select[name=m_grade] option:eq("+m_grade+")").attr("selected","selected");
		...
		function check() {
			if(filetype == '' || filetype == null){
				$('input[name=m_file2]').val('${movieVO.m_file}');
			}
